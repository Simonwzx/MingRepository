...
public final class CRC16Util {
	...
	static short crc16Ccitt(byte [] pSrcData) {
		int crcReg = 0x0000;
		for (byte pSrcDatum : pSrcData) {
			crcReg = CRC16_ccitt_table[(crcReg ^ pSrcDatum) & 0xFF] ^ (crcReg >> 8);
		}
		return (short) crcReg;
	}

	public static void main(String[] args) {
		byte [] bytes = new byte []{0x31,0x32,0x33,0x34,0x39};
		short re = crc16Ccitt(bytes);
		String hexStr = PacketDecoderUtils.bytes2HexStr(CommonUtils.short2Bytes(re));
